...
test('Empresas alterna entre cadastro e organograma, e um vínculo volta à ficha',()=>{const a=app();a.run('tela()');a.document.querySelector('[data-view=organograma]').click();assert.equal(a.run('filtro.empresaAba'),'organograma');assert.equal(a.document.querySelector('#empresas-organograma').hidden,false);assert.ok(a.document.querySelector('#empresas-organograma').textContent.includes('Grupo Exemplo'));a.run("empresaAbrir('empresa-a')");assert.equal(a.run('filtro.empresaAba'),'cadastros');assert.equal(a.document.querySelector('.empresa-contexto h2').textContent,'Empresa Exemplo');});
