...
test('leitura de datas: vencimento explícito gera evento pronto',()=>{const x=extractDates(mail('Vencimento: 10/09/2026'),today);assert.equal(x[0].date,'2026-09-10');assert.equal(x[0].status,'ready')});
test('leitura de datas: renovação por extenso é reconhecida',()=>{const x=extractDates(mail('Renovação em 10 de setembro de 2026'),today);assert.equal(x[0].date,'2026-09-10');assert.equal(x[0].kind,'Renovação');assert.equal(x[0].status,'ready')});
test('leitura de datas: compromisso ISO é reconhecido',()=>assert.equal(extractDates(mail('Reunião: 2026-09-12'),today)[0].status,'ready'));
...
test('leitura de datas: data sem contexto de compromisso não vira evento',()=>assert.equal(extractDates(mail('Nossa empresa foi fundada em 10/09/2026'),today).length,0));
...
test('leitura de datas: busca usa intervalo fechado entre execuções',()=>{const q=gmailQuery(100,200);assert.match(q,/after:100 before:200/);assert.match(q,/renova/);assert.match(q,/-in:spam/)});
