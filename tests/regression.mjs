...
test('evento às 23h30 termina no próximo dia',async()=>{const run=app();const ics=await run("E.agenda=[{id:'1',data:'2026-09-05',hora:'23:30',titulo:'Evento'}];var arquivo;baixar=b=>arquivo=b;exportarIcs();arquivo.text()");assert.equal(new ICAL.Component(ICAL.parse(ics)).getFirstSubcomponent('vevent').getFirstPropertyValue('dtend').toJSDate().toISOString(),'2026-09-06T03:30:00.000Z')});
