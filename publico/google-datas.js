...
function extractDates(msg,today=day(Date.now())){
 ...
 const items=[];const seen=new Set(),keys=new Set();
 for(const line of lines){
  const text=fold(line),kind=/renov|vigencia/.test(text)?'Renovação':/venc|boleto|fatura|pagamento|parcela|cobranca/.test(text)?'Financeiro':/compromisso|reuniao|consulta|agendad|audiencia|evento|entrevista|reserva|entrega|prazo/.test(text)?'Compromisso':'Data encontrada';
  ...
  for(const m of matches){
   ...
   if(!dateOK(date)||date<today||seen.has(kind+date+line))continue;seen.add(kind+date+line);
   const baseKey=`mail:${msg.id}:${kind}:${date}`,key=keys.has(baseKey)?baseKey+':'+encodeURIComponent(line):baseKey;keys.add(baseKey);
   items.push({key,title:`${kind} · ${title}`,date,kind,status:'review',sourceId:msg.id,sourceVersion:Number(msg.internalDate)||0,excerpt:line.slice(0,400),sender,reason:'Data encontrada no e-mail; confira o contexto e aprove antes de importar'});
  }
