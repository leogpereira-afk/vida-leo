...
(function (global) {
  ...
  function sociosResumidos(valor){
    return texto(valor).split(/[;\n]+|\s*\/\s*|(?<=%)\s*,?\s*(?=[A-Za-zÀ-ÿ])/u).map(texto).filter(Boolean).map(parte=>{
      const percentual=parte.match(/(\d+(?:[.,]\d+)?)\s*%/g)||[];
      const nome=parte.replace(/\d+(?:[.,]\d+)?\s*%/g,'').replace(/^[\s:–—()\-]+|[\s:–—()\-]+$/g,'').trim();
      if(percentual.length>1||/\s(?:e|&)\s/i.test(nome)||!nome)return 'Conferir composição no cadastro';
      return nome.split(/\s+/)[0]+' · '+(percentual[0]||'% não informado');
    }).join('\n');
  }
  ...
  function moverNo(o,noId,direcao) {
    if(![-1,1].includes(direcao))throw Error('Direção inválida.');
    const novo=clone(o),n=novo.nos.find(x=>x.id===noId);
    if(!n)throw Error('Item não encontrado.');
    const irmaos=filhos(novo,n.parentId||''),pos=irmaos.findIndex(x=>x.id===noId),destino=pos+direcao;
    if(destino<0||destino>=irmaos.length)return novo;
    [irmaos[pos],irmaos[destino]]=[irmaos[destino],irmaos[pos]];
    irmaos.forEach((x,i)=>x.ordem=i);
    validar([novo]);novo.atualizadoEm=new Date().toISOString();return novo;
  }
  ...
  function cartaoExportacao(n, estado, notas, ctx) {
    ...
    for (const [chave,rotulo] of [['cnpj','CNPJ'],['regime','Regime'],['banco','Banco'],['contabilidade','Contabilidade']]) {const valor=['cnpj','regime','contabilidade'].includes(chave)?info[chave]:n[chave];if(valor)blocos.push({rotulo,texto:valor,linhas:linhas(valor)});}
    if (n.atividade) blocos.push({rotulo:'Atividade',texto:n.atividade,linhas:linhas(n.atividade)});
    if (info.socios) blocos.push({rotulo:'Sócios e participação',texto:info.socios,linhas:linhas(info.socios)});
  ...
  function diagramaSVG(o, estado, opts = {}, ctx = {}) {
    ...
    const cores = escuro ? {fundo:'#191e29',linha:'#455064',tinta:'#f5f7fa',suave:'#bbc4d2',destaque:'#a5adff'} : {fundo:'white',linha:'#d8e3df',tinta:'#21332d',suave:'#68796f',destaque:'#3e45ee'};
    ...
    function medir(n, nivel) {
      ...
      if(opts.resumo){cartao.blocos=cartao.blocos.filter(b=>['Atividade','CNPJ','Regime','Contabilidade','Sócios e participação'].includes(b.rotulo)).map(b=>({...b,linhas:b.rotulo==='Atividade'&&b.linhas.length>2?[b.linhas[0],b.linhas[1]+'…']:b.linhas}));cartao.altura=Math.max(cartao.info.logo?100:0,60+cartao.nome.length*19+cartao.blocos.reduce((v,b)=>v+28+b.linhas.length*16,0));}
    ...
    if(opts.resumo&&opts.horizontal){
      const rotulos=['CNPJ','Regime','Contabilidade','Sócios e participação'];
      const nomes=Math.max(2,...[...dados.values()].map(d=>d.cartao.nome.length));
      for(const d of dados.values()){
        while(d.cartao.nome.length<nomes)d.cartao.nome.push('');
        const blocos=d.cartao.blocos;d.cartao.blocos=rotulos.map(rotulo=>blocos.find(b=>b.rotulo===rotulo)||{rotulo,linhas:['Não informado']}).concat(blocos.filter(b=>!rotulos.includes(b.rotulo)));
      }
  ...
  function htmlExportacao(original, estado, opts = {}, ctx = {}) {
    ...
    const somenteDesenho=opts.formato==='visual';
  ...
  function render(container, ctx) {
    ...
    function editarNo(o,n,paiId='') {
      ...
      abrir(n?'Editar item':'Adicionar item','A ligação organiza o desenho. Ela não define participação societária.',(c,fechar)=>{
        ...
        const atualizarLogo=()=>{const info=identificar({...n,empresaId:controles.empresaId.value,logoPersonalizada},state(),ctx);previa.hidden=!info.logo;if(info.logo)previa.src=info.logo;else previa.removeAttribute('src')};
        arquivo.onchange=()=>{const atual=++leitura,file=arquivo.files?.[0];carregando=false;erro.textContent='';if(!file)return;
          if(!['image/png','image/jpeg','image/webp'].includes(file.type)||file.size>1024*1024){erro.textContent='Escolha PNG, JPEG ou WebP de até 1 MB.';arquivo.value='';return}
          carregando=true;const reader=new global.FileReader();reader.onload=()=>{if(atual!==leitura||!f.isConnected)return;carregando=false;const valor=String(reader.result||'');if(!logoSegura(valor)){erro.textContent='Não foi possível ler esta imagem.';return}logoPersonalizada=valor;atualizarLogo()};reader.onerror=()=>{if(atual!==leitura)return;carregando=false;erro.textContent='Não foi possível ler esta imagem. Tente novamente.'};reader.readAsDataURL(file);
        };
        bloco.append(rotulo,criar('small','','PNG, JPEG ou WebP, até 1 MB. A alteração será aplicada somente ao salvar este card.'),previa,botao('Usar logo do cadastro',()=>{leitura++;carregando=false;logoPersonalizada='';arquivo.value='';erro.textContent='';atualizarLogo()}));f.insertBefore(bloco,f.children[2]);atualizarLogo();
        let empresaAnterior=empresas.find(e=>e.id===n?.empresaId);
        controles.empresaId.onchange=()=>{const e=empresas.find(e=>e.id===controles.empresaId.value);if(e&&(!texto(controles.nome.value)||controles.nome.value===empresaAnterior?.nome))controles.nome.value=e.nome;empresaAnterior=e;atualizarLogo();};
      });
    }
    ...
    function exportar(o) {
      abrir('Exportar organograma em PDF','Escolha o desenho completo ou apenas uma parte. A exportação abre em outra janela.',(c,fechar)=>{
        ...
        f.prepend(criar('p','org-campo-full org-ajuda','Na prévia, clique em “Salvar em PDF / Imprimir”. O formato padrão inclui apenas o título e o desenho, em uma página A3 na horizontal. Para incluir dados e observações, escolha outro formato.'));
      });
    }
    ...
    function pintar() {
      ...
      if(!o.nos.length){const vazio=criar('div','org-vazio');vazio.append(criar('p','','Adicione o primeiro item no topo da estrutura.'),botao('Adicionar primeiro item',()=>editarNo(o),''));painel.append(vazio)}
        ...
        function ramo(n){
          ...
          for(const [chave,rotulo]of [['cnpj','CNPJ'],['regime','Regime'],['banco','Banco'],['contabilidade','Contabilidade']]){const valor=['cnpj','regime','contabilidade'].includes(chave)?info[chave]:n[chave];if(valor){const dado=criar('p','org-no-referencia');dado.append(criar('b','',rotulo+': '),doc.createTextNode(valor));cartao.append(dado)}}
          if(n.atividade)cartao.append(criar('p','org-no-atividade',n.atividade));
          if(info.socios){const d=criar('div','org-no-dado');d.append(criar('small','','Sócios e participação'),criar('p','',info.socios));cartao.append(d)}
          ...
          if(modo==='editar'){
            if(n.parentId)acoes.append(botao('Separar',()=>{try{mudar(salvarNo(o,{...n,parentId:'',ordem:filhos(o).length}),o,JSON.stringify(o))}catch(e){cartao.append(criar('p','org-erro',e.message))}}));
            const irmaos=filhos(o,n.parentId||''),pos=irmaos.findIndex(x=>x.id===n.id);
            for(const [direcao,rotulo]of [[-1,'← Esquerda'],[1,'Direita →']]){
              const b=botao(rotulo,()=>{try{mudar(moverNo(o,n.id,direcao),o,JSON.stringify(o))}catch(e){cartao.append(criar('p','org-erro',e.message))}});
              b.disabled=pos+direcao<0||pos+direcao>=irmaos.length;b.setAttribute('aria-label',rotulo+' '+info.nome+' na ordem');acoes.append(b);
            }
          }
  ...
  global.LeoOrganograma={render,validar,salvarNo,removerNo,moverNo,listaImportacao,importarNoEstado,identificar,recorte,diagramaSVG,htmlExportacao,sociosResumidos,otimizarLogo,otimizarOrganogramas};
})(globalThis);
